...
app.post('/download-audio', (req, res) => {
  ...
  // -x: extract audio, --audio-format mp3: convert to mp3
...
app.post('/download-video-only', (req, res) => {
  ...
  // Generate a unique filename for the video-only file using a timestamp
  const fileName = `video-${Date.now()}.mp4`;
  const outputFilePath = path.join(__dirname, 'downloads', fileName);

  // Build the yt-dlp command to download just the video stream.
  // Use "-f bestvideo" to select the best available video without audio.
  // Using --merge-output-format mp4 to force conversion to MP4 if necessary.
  const command = `${ytDlpPath} --no-check-certificate -f bestvideo --merge-output-format mp4 -o "${outputFilePath}" "${url}"`;
  console.log(`Executing command: ${command}`);
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    setTimeout(() => {
      if (!fs.existsSync(outputFilePath)) {
        console.error('Video-only file does not exist at path:', outputFilePath);
